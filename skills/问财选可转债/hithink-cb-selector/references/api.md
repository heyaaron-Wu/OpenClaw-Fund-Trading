...
查询可转债相关金融数据，支持自然语言问句输入，返回相关金融数据结果。
...
| 参数名 | 类型 | 必填 | 说明 |
|--------|------|------|------|
| query | STRING | 是 | 用户问句，例如："转股溢价率低于10%的可转债有哪些？" |
...
**分页说明：**
- `datas` 默认只返回10条数据，但符合条件的可转债总数 `code_count` 可能远大于此
...
- 当用户询问 **可转债筛选、转股溢价率、评级、剩余期限** 等相关问题时，调用本接口。
- 将用户的自然语言问题直接作为 `query` 参数传入。
- 返回的 `datas` 字段为对象数组，每个对象包含可转债代码(`可转债代码`)、可转债简称(`可转债简称`)、转股溢价率(`转股溢价率`)等字段，可直接拼接后回复用户。
