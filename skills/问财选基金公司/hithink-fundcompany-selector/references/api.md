...
查询基金公司相关金融数据，支持自然语言问句输入，返回相关金融数据结果。
...
| 参数名 | 类型 | 必填 | 说明 |
|--------|------|------|------|
| query | STRING | 是 | 用户问句，例如："规模最大的基金公司有哪些？" |
...
**分页说明：**
- `datas` 默认只返回10条数据，但符合条件的基金公司总数 `code_count` 可能远大于此
...
- 当用户询问 **基金公司筛选、基金公司规模、基金公司排名、优秀基金公司** 等相关问题时，调用本接口。
- 将用户的自然语言问题直接作为 `query` 参数传入。
- 返回的 `datas` 字段为对象数组，每个对象包含基金公司名称(`基金公司名称`)、管理规模(`管理规模`)、旗下基金数量(`旗下基金数量`)等字段，可直接拼接后回复用户。
