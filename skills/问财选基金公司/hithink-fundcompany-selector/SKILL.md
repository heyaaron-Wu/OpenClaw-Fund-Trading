---
name: 问财选基金公司
description: 同花顺智能选基金公司skill。根据管理规模、旗下产品业绩、投研实力、风险评级等维度筛选公募基金公司。返回符合条件的相关基金公司数据。当用户询问基金公司筛选问题时，必须使用此技能。
---
...
本技能提供公募基金公司智能筛选能力，通过自然语言查询支持：
- 管理规模筛选（资产规模、百亿规模、千亿规模等）
- 产品业绩筛选（基金收益排名、业绩稳定性等）
- 投研实力筛选（投研团队、获奖数量、评级等）
- 风险评级筛选（风控能力、风险评级等）
- 产品线筛选（基金产品数量、基金类型覆盖等）
- 多条件组合筛选
...
**改写规则：**
- 保留用户核心意图（如：规模排名前十、业绩最好等）
- 将口语化表达转为标准金融术语（如"帮我选规模大的" → "管理规模排名前十的基金公司"）
- 适当简化过于复杂的复合条件
- 改写后需保持原意不变
...
调用金融查询接口获取数据，支持分页参数。注意：默认返回10条数据，但符合条件的基金公司总数可能更多，需关注 `code_count` 字段并通过分页获取全部数据。
...
# 解析返回数据
datas = result.get("datas", [])           # 当前页基金公司列表
code_count = result.get("code_count", 0)  # 符合条件的总基金公司数
...
```python
for item in datas:
    # 根据查询类型提取相应字段，如基金公司名称、管理规模、旗下基金数量等
```

### 步骤 6: 数据扩展决策

skill 需要自行决策当前数据是否足够回答用户问题：
- 如果数据完整：直接返回格式化后的结果且保证选基金公司表格正确解析为表格展示
...
| 参数名 | 类型 | 说明 |
|--------|------|------|
| datas | ARRAY | 金融数据列表，对象数组，每个对象包含基金公司名称、管理规模、旗下基金数量等字段 |
| code_count | INT | 符合查询条件的总基金公司数量（注意：可能大于当前返回的datas条数） |
...
{
  "datas": [
    {"基金公司名称": "易方达基金", "管理规模": "15000亿", "旗下基金数量": 320},
    {"基金公司名称": "广发基金", "管理规模": "12000亿", "旗下基金数量": 280}
  ],
  "code_count": 20,
  "chunks_info": {
    "query": "规模排名前十的基金公司",
    "parsed_conditions": ["管理规模排名前十"]
  }
}
```
...
- `code_count` 表示符合条件的总基金公司数，可能远大于 `datas` 的长度
- 当 `code_count > len(datas)` 时，需要通过 `page` 参数翻页获取更多数据
- 返回的表格数据需要解析 `datas` 数组中的对象字段，如 `基金公司名称`、`管理规模`、`旗下基金数量` 等
...
```bash
# 直接查询（默认返回10条，注意：符合条件的基金公司总数可能更多）
python3 scripts/cli.py --query "规模最大的基金公司有哪些？"

# 指定分页参数（page从1开始，limit为每页条数）
python3 scripts/cli.py --query "业绩最好的基金公司" --page "1" --limit "20"

# 指定API密钥
python3 scripts/cli.py --query "头部基金公司" --api-key "your-key"
```

**分页说明：**
- 默认返回10条数据，但符合条件的基金公司总数（code_count）可能远大于10
- 当 code_count > 返回条数时，表示还有更多数据，可通过 `--page` 参数翻页获取
- 例如：code_count=20，limit=10，则需要翻2页才能获取全部数据
